...
func setPaths() {
	var logFile, logPsqlFile, portStr string

	portStr = "\"-p " + strconv.Itoa(conf.Port) + "\""
	if len(conf.UsedVersion) > 0 {
		...
		cmdInitDbArgs = []string{"-D", dataDir, "-U", conf.Username, "-A", "trust", "-E", "UTF8", "--locale", conf.Locale, "-k", "-n"}
		cmdStartArgs = []string{"-o", portStr, "-D", dataDir, "-l", logFile, "-w", "start"}
		cmdStopArgs = []string{"-D", dataDir, "stop"}
		cmdStatusArgs = []string{"-D", dataDir, "status"}
		cmdStartShellArgs = []string{"/C", "start", "/wait", pgShell, "-L", logPsqlFile, "-U", conf.Username, conf.Username}
